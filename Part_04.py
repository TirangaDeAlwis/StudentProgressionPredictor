...
def startQ4():
    ...
    dataFile = open("CreditDataSheet.txt","w")
    dataFile.write("")
    dataFile.close()
...
def progressionOutcome(passCr, deferCr, failCr): 
    global progressCount, retrieverCount, trailerCount, excludeCount, totalCount    # declare varible as global
    
    if passCr == 120:
        resultList.append(['Progress', passCr, deferCr, failCr])
        writeData("progress - " + str(passCr) + ", " + str(deferCr) + ", " + str(failCr) + "\n")
        progressCount += 1   # increment variable value by 1
        print("Progress") 
    elif passCr == 100:
        resultList.append(['Progress (module trailer)', passCr, deferCr, failCr])
        writeData("Progress (module trailer) - " + str(passCr) + ", " + str(deferCr) + ", " + str(failCr) + "\n")
        ...
        writeData("Exclude - " + str(passCr) + ", " + str(deferCr) + ", " + str(failCr) + "\n")
        excludeCount += 1  # increment variable value by 1
        print("Exclude")   
    else:
        resultList.append(['Module retriever', passCr, deferCr, failCr])
        writeData("Module retriever - " + str(passCr) + ", " + str(deferCr) + ", " + str(failCr) + "\n")
...
#save input progression data to a text file    
def writeData(data):
    dataFile = open("CreditDataSheet.txt","a")
    dataFile.write(data)
    dataFile.close()

#Import data from the text file    
def showInfoFromFile():
    print("Data from file")
    dataFile = open("CreditDataSheet.txt","r")
    print(dataFile.read())
    print(f'{totalCount} outcomes in total.')
    print('-' * 70)
